...
def style_logo_colors(logo, sequence_length, wildtype, intended, wt_color='grey', mut_color='red'):
    ...
    for pos in range(sequence_length):
        ...
        # Set wildtype to specified color
        logo.style_single_glyph(p=pos, c=wt, color=wt_color)
        
        # Set mutations to specified color
...
def mutant_dict_to_string(mutant_dict, all_mutations, intended, threshold_ratio=0.6, use_dots=True):
    """
    ...
    # Convert sequences from dot notation to intended sequences
    converted_seqs, wildtype = convert_dots_to_intended_sequences(mutant_seqs, all_mutations, intended)
    ...
    logo_string = counts_to_string(counts_mat, threshold_ratio=threshold_ratio, wildtype=wildtype, use_dots=use_dots)
...
def plot_logo(mutant_seqs, all_mutations, intended, threshold_ratio=0.6, figsize=(10,2), save_path=None):
...
if __name__ == "__main__":
    mutant_dict_file = "/work/greencenter/s439821/TEM1CML/data/processed/node_group_mutants.json"
    with open(mutant_dict_file, 'r') as f:
        mutant_dict = json.load(f)

    logo_string = mutant_dict_to_string(mutant_dict, all_mutations, intended, threshold_ratio=0.6, use_dots=True)
    print(logo_string)

    filename = "amp_3_1_big.png"
    save_path = f"/work/greencenter/s439821/TEM1CML/data/outputs/figures/logo/graph_{filename}"
    mutant_seqs = list(mutant_dict.keys())
    fig = plot_logo(mutant_seqs, all_mutations, intended, threshold_ratio=0.6, figsize=(8,2), save_path=save_path)
    plt.show()
