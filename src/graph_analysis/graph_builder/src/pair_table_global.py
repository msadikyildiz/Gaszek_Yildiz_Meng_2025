...
def calculate_normalized_fitness(long_df):
    ...
    auc_df = (long_df
        ...
        .with_columns([
            ...
            .map_elements(lambda x: np.log10(
                np.trapz(y=np.power(10, np.array(x['replicate1'])), 
                        x=np.array(x['concentration']))
            ) if not np.all(np.isnan(x['replicate1'])) else np.nan,
            return_dtype=pl.Float64)
            .alias('auc1'),
            
            pl.struct(['replicate2', 'concentration'])
            .map_elements(lambda x: np.log10(
                np.trapz(y=np.power(10, np.array(x['replicate2'])), 
                        x=np.array(x['concentration']))
            ) if not np.all(np.isnan(x['replicate2'])) else np.nan,
            return_dtype=pl.Float64)
            .alias('auc2'),
            
            pl.struct(['replicate3', 'concentration'])
            .map_elements(lambda x: np.log10(
                np.trapz(y=np.power(10, np.array(x['replicate3'])), 
                        x=np.array(x['concentration']))
            ) if not np.all(np.isnan(x['replicate3'])) else np.nan,
            return_dtype=pl.Float64)
            .alias('auc3')
        ])
        ...
        .with_columns([
            ...
            .map_elements(lambda x: np.nanmean(x), return_dtype=pl.Float64) # consistent with Erdal's 
            .alias('fitness'),
...
def get_pairs(global_fitness_df):
    ...
    for m1, m2 in mutant_pairs:
        m1_fitness = global_fitness_df.filter(pl.col('mutant_profile') == m1)['fitness'].item()
        m2_fitness = global_fitness_df.filter(pl.col('mutant_profile') == m2)['fitness'].item()
...
def main():
    ...
    # Save results
    amp_global_fitness_df.sort("mutant_profile").write_csv(f"{base_path}/data/processed/amp_global_fitness.csv")
    azt_global_fitness_df.sort("mutant_profile").write_csv(f"{base_path}/data/processed/azt_global_fitness.csv")

    amp_pairs.write_csv(f"{base_path}/data/processed/amp_global_pairs.csv")
    azt_pairs.write_csv(f"{base_path}/data/processed/azt_global_pairs.csv")
    print("results saved")
