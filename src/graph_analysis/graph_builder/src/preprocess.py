...
def preprocess_data(amp_path, azt_path, amp_pairs_path, azt_pairs_path, clean_nulls_flag):
    """
    ...
    amp_long_df = amp_long_df.with_columns(
        ...
        .map_elements(lambda s: np.nanmedian([s["replicate1"], s["replicate2"], s["replicate3"]]), return_dtype=pl.Float64).alias('median')
    )
...
def clean_nulls(df, num_rep_threshold=2):
    ...
    threshold = (df.shape[1] - 3) * num_rep_threshold / 3  # delete rows with more than num_rep_threshold replicates missing
