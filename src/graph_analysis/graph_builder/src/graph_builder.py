...
from src.make_logo import mutant_dict_to_string, all_mutations, intended
...
class GraphBuilder:
    ...
    def __init__(
        ...
        self.initial_graph: Optional[nx.MultiDiGraph] = None
        self.neutral_merged_graph: Optional[nx.MultiDiGraph] = None
        self.peak_graph: Optional[nx.DiGraph] = None
    ...
    def build_raw_graph(
        ...
        if concentration is not None:
            table_nodes = self.long_table.filter(pl.col("concentration") == concentration)
            table_edges = self.pairwise_table.filter(pl.col("concentration") == concentration)
        else:
    ...
    def merge_neutral_edges(
        ...
        forbidden_pairs: Optional[Set[frozenset]] = None
    ) -> nx.MultiDiGraph:
        """
        ...
        def would_merge_forbidden(u: str, v: str) -> bool:
            ...
            for fset in forbidden_pairs:
                if len(fset) != 2:
                    logger.info(f"skipping forbidden pair {fset} because it's not a pair")
                    continue
        ...
        logger.info(f"Total pairs prevented from merging due to forbidden constraints: {len(skipped_merges)}")
        ...
        for (ru, rv), forward_data in edge_map.items():
            ...
            if net_flow > 0:
                merged_graph.add_edge(ru, rv, weight=net_flow, count=total_count, forbidden=int(is_forbidden))
            elif net_flow < 0:
                merged_graph.add_edge(rv, ru, weight=abs(net_flow), count=total_count, forbidden=int(is_forbidden))
    ...
    def merge_peaks(
        ...
        if use_parallel:
            with get_context("spawn").Pool() as pool:
                peak_anc_map = dict(
                    pool.map(
                        partial(self._gather_ancestors_worker, predecessor_map),
                        peaks
                    )
                )
        else:
            ...
            for peak_node in peaks:
                peak_anc_map[peak_node], ancestors = self._gather_ancestors_worker(predecessor_map, peak_node)
        ...
        if use_parallel:
            with get_context("spawn").Pool() as pool:
                peak_supernodes = dict(pool.map(self._merge_single_peak_group, peak_data_list))
        else:
            peak_supernodes = {}
        ...
        for (source, target) in merged_graph.edges():
            merged_graph[source][target]["deterministic"] = int(merged_graph.nodes[target].get("is_peak", 0))

        return merged_graph
        
    def mark_peaks_and_edges(
        self,
        input_graph: nx.MultiDiGraph
    ) -> nx.DiGraph:
        """
    ...
    ############################################################
    def rename_nodes_closest_to_average(
        self,
        ...
        if rename_map:
            ...
            logger.info(f"Renamed {len(rename_map)} nodes to their closest-to-average mutants.")
    ...
    def update_peak_nodes_max_fitness(self):
        ...
        if nodes_to_rename:
            self.peak_graph = nx.relabel_nodes(self.peak_graph, nodes_to_rename, copy=False)
            logger.info(f"Renamed {len(nodes_to_rename)} peak nodes")

    def get_forbidden_pairs(
        self,
        graph: nx.MultiDiGraph,
        num_forbidden_pairs: Optional[int] = None,
        large_edge_threshold: Optional[float] = None
    ) -> Set[frozenset]:
        """
        ...
        sorted_threshold_pairs = sorted(threshold_pairs.items(), key=lambda x: x[1], reverse=True)
        logger.info(f"Found {len(sorted_threshold_pairs)} pairs with weight >= {large_edge_threshold}:")
    ...
    def build_graph(
        ...
        large_edge_threshold: Optional[float] = 5.5,
        num_forbidden_pairs: Optional[int] = None,
        tiny_initial_threshold: float = 0.02,
        merge_peaks: bool = True,
        logo_threshold_ratio: float = 0.6
    ) -> nx.DiGraph:
        logger.info(f"Starting build_graph, concentration={concentration}, "
        ...
        logger.info(f"Initial raw graph -> "
        ...
        logger.info(f"After tiny merge -> "
        ...
        if forbidden_pairs:
            logger.info(f"Proceeding with {len(forbidden_pairs)} forbidden pairs for neutral merging.")
        ...
        logger.info(f"After merge neutral edges -> "
        ...
        logger.info(f"After peak processing -> "
        ...
        for node in self.peak_graph.nodes():
            ...
            self.peak_graph.nodes[node]["fitness[z]"] = self.peak_graph.nodes[node]["fitness"]
